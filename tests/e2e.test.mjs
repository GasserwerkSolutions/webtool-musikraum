...
test("real browser layout, navigation and sidebar contract", { timeout: 90000 }, async () => {
  ...
  try {
    ...
    const desktop = await page.evaluate(() => { const frame = document.querySelector("#previewFrame"); const desk = document.querySelector(".preview-desk"); const header = document.querySelector(".topbar"); const redo = document.querySelector("#redoButton"); const desktopButton = document.querySelector('[data-viewport="desktop"]'); const mobileButton = document.querySelector('[data-viewport="mobile"]'); const reset = document.querySelector('[data-action="reset"]'); const frameRect = frame.getBoundingClientRect(); const deskRect = desk.getBoundingClientRect(); return { frameWidth: frameRect.width, frameTop: frameRect.top, frameRight: frameRect.right, deskWidth: deskRect.width, headerBottom: header.getBoundingClientRect().bottom, outerVertical: desk.scrollHeight - desk.clientHeight, bodyVertical: document.documentElement.scrollHeight - document.documentElement.clientHeight, bodyHorizontal: document.documentElement.scrollWidth - document.documentElement.clientWidth, toolbarExists: Boolean(document.querySelector(".preview-toolbar")), controlsOrdered: redo.getBoundingClientRect().right <= desktopButton.getBoundingClientRect().left && mobileButton.getBoundingClientRect().right <= reset.getBoundingClientRect().left }; });
    assert.ok(desktop.frameWidth <= 1200); assert.ok(desktop.frameWidth <= desktop.deskWidth); assert.ok(Math.abs(desktop.frameTop - desktop.headerBottom) <= 1); assert.ok(desktop.frameRight <= 1440); assert.equal(desktop.toolbarExists, false); assert.equal(desktop.controlsOrdered, true); assert.equal(desktop.outerVertical, 0); assert.ok(desktop.bodyVertical <= 1); assert.ok(desktop.bodyHorizontal <= 1); assert.ok(await preview.evaluate(() => document.documentElement.scrollHeight > innerHeight));
