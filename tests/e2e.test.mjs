...
const MIME = { ".html": "text/html", ".js": "text/javascript", ".mjs": "text/javascript", ".css": "text/css", ".svg": "image/svg+xml", ".webp": "image/webp" };
...
test("real browser layout, live editing and sidebar contract", { timeout: 90000 }, async () => {
  ...
  try {
    ...
    await page.click('[data-viewport="tablet"]'); await page.waitForFunction(() => Math.abs(document.querySelector("#previewFrame")?.getBoundingClientRect().width - 768) < .1); assert.equal(await page.$eval('[data-viewport="tablet"]', (button) => button.getAttribute("aria-pressed")), "true");
    ...
    await page.click('[data-viewport="mobile"]'); await page.waitForFunction(() => Math.abs(document.querySelector("#previewFrame")?.getBoundingClientRect().width - 390) < .1); await preview.click(".menu-button"); assert.equal(await preview.$eval(".main-nav", (nav) => nav.classList.contains("is-open")), true); await preview.click(".menu-button");
    ...
    await page.focus("#sidebarResizer"); await page.keyboard.press("Home"); await page.keyboard.press("ArrowRight"); const storedWidth = await page.$eval(".control-surface", (surface) => surface.getBoundingClientRect().width); await page.reload({ waitUntil: "domcontentloaded" }); await page.waitForFunction(() => ["saved", "session"].includes(document.querySelector("#saveStatus")?.dataset.state)); const restoredWidth = await page.$eval(".control-surface", (surface) => surface.getBoundingClientRect().width); assert.ok(Math.abs(restoredWidth - storedWidth) <= 1);
