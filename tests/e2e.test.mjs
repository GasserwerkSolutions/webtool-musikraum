...
test("real browser layout, live editing and sidebar contract", { timeout: 90000 }, async () => {
  ...
  try {
    ...
    assert.equal(await page.$eval('#heroPointList [data-text-item-card]:first-child [data-text-item-field]', (input) => input.value), heroOrderBefore[1]); await preview.waitForFunction((value) => document.querySelector(".hero-notes span")?.textContent.includes(value), {}, heroOrderBefore[1]); await preview.waitForFunction((value) => document.querySelector(".hero-notes span")?.textContent.includes(value), {}, heroOrderBefore[1]);
    ...
    assert.deepEqual(await page.$$eval('#heroPointList [data-text-item-field]', (inputs) => inputs.map((input) => input.value)), heroOrderBefore); await preview.waitForFunction((value) => document.querySelector(".hero-notes span")?.textContent.includes(value), {}, heroOrderBefore[0]); await preview.waitForFunction((value) => document.querySelector(".hero-notes span")?.textContent.includes(value), {}, heroOrderBefore[0]);
    ...
    assert.equal(await page.$eval('#offerList [data-offer-card]:first-child [data-offer-field="title"]', (input) => input.value), offerOrderBefore[1]); await preview.waitForFunction((value) => document.querySelector(".card h3")?.textContent.includes(value), {}, offerOrderBefore[1]); await preview.waitForFunction((value) => document.querySelector(".card h3")?.textContent.includes(value), {}, offerOrderBefore[1]);
    assert.match(await page.$eval('[data-action="undo"]', (button) => button.getAttribute("aria-label") ?? ""), /Klangmoment .* verschoben/);
    await page.click('[data-action="undo"]'); assert.deepEqual(await page.$$eval('#offerList [data-offer-field="title"]', (inputs) => inputs.map((input) => input.value)), offerOrderBefore); await preview.waitForFunction((value) => document.querySelector(".card h3")?.textContent.includes(value), {}, offerOrderBefore[0]); await preview.waitForFunction((value) => document.querySelector(".card h3")?.textContent.includes(value), {}, offerOrderBefore[0]);
