...
test("real browser layout, live editing and sidebar contract", { timeout: 90000 }, async () => {
  ...
  try {
    ...
    const footer = await exportedPage.evaluate(() => { const address = document.querySelector(".footer-address")?.getBoundingClientRect(); const email = document.querySelector(".footer-email")?.getBoundingClientRect(); return { separate: Boolean(address && email && email.top >= address.bottom), text: document.querySelector(".footer-contact")?.textContent ?? "" }; }); assert.equal(footer.separate, true); assert.doesNotMatch(footer.text, /Brügginfo@/); await exportedPage.close();
