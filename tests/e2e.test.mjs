...
test("real browser layout, navigation and sidebar contract", { timeout: 90000 }, async () => {
  ...
  try {
    ...
    const desktop = await page.evaluate(() => { const frame = document.querySelector("#previewFrame"); const desk = document.querySelector(".preview-desk"); const header = document.querySelector(".topbar"); const viewportSwitch = document.querySelector(".viewport-switch--topbar"); const saveStatus = document.querySelector("#saveStatus"); const frameRect = frame.getBoundingClientRect(); const deskRect = desk.getBoundingClientRect(); return { frameWidth: frameRect.width, frameTop: frameRect.top, frameRight: frameRect.right, deskWidth: deskRect.width, headerBottom: header.getBoundingClientRect().bottom, borderRadius: getComputedStyle(frame).borderRadius, outerVertical: desk.scrollHeight - desk.clientHeight, bodyVertical: document.documentElement.scrollHeight - document.documentElement.clientHeight, bodyHorizontal: document.documentElement.scrollWidth - document.documentElement.clientWidth, toolbarExists: Boolean(document.querySelector(".preview-toolbar")), controlsOrdered: viewportSwitch.getBoundingClientRect().right <= saveStatus.getBoundingClientRect().left }; });
