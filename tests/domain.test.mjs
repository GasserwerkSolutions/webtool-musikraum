...
describe("Raum für Klang domain", () => {
  test("creates a focused Raum für Klang draft", () => {
    const draft = createDefaultDraft("2026-08-08T12:00:00.000Z");
    assert.equal(draft.schemaVersion, 1);
    assert.equal(draft.site.name, "Raum für Klang");
    assert.equal(draft.site.tagline, "Gemeinsam spielen. Aufeinander hören. Klang entstehen lassen.");
    assert.equal(draft.site.phone, "+41 79 675 58 69");
    assert.equal(draft.site.email, "info@Musikraum.ch");
  ...
  test("normalizes section order, visibility and safe values", () => {
    ...
    assert.equal(normalized.theme.primary, "#214f68");
  });
  ...
  test("website follows Franz chosen order and emits structured offer data", () => {
    ...
    assert.match(html, /Instrumente entdecken/);
  ...
  test("prepares three explicit photo slots with incontext SVG placeholders", () => {
    const html = buildWebsiteHtml(createDefaultDraft());
    assert.equal((html.match(/data-photo-slot=/g) ?? []).length, 3);
    assert.match(html, /data-photo-slot="sandpendel"/);
    assert.match(html, /data-photo-slot="franz-portrait"/);
    assert.match(html, /data-photo-slot="instrument-detail"/);
    assert.match(html, /Bildplatzhalter/);
  });
});
