...
async function handleExport(context) {
    ...
    if (!readiness.ready) {
        showToast(`${readiness.errorCount} ${readiness.errorCount === 1 ? "Blocker muss" : "Blocker müssen"} vor dem Export behoben werden.`);
        return;
    }
    const controller = context.exportController;
    if (!controller) {
        showToast("Die Exportvorbereitung ist noch nicht verfügbar.");
        return;
    }
    ...
    if (state.status === "ready")
        showToast("Export vorbereitet. Prüfe die Angaben und klicke danach auf „HTML-Datei herunterladen“.");
    else if (state.status === "failed" && readiness.ready)
        showToast(state.message);
}
function downloadBlob(blob, filename) { const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = filename; document.body.appendChild(link); link.click(); link.remove(); setTimeout(() => URL.revokeObjectURL(url), 1_000); }
