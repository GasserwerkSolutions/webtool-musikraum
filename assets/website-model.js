import { FONT_PRESETS, FONT_SIZES, PRESETS, } from "./domain.js";
...
export function websiteModelFromDraft(draft) {
...
export function websiteModelFromResolved(page, resolved) {
    ...
    const visibility = Object.fromEntries(SECTION_KEYS.map((key) => [key, section(page, key).enabled]));
    return {
        site: {
            name: text(site.name),
...
export function mediaFromResolved(resolved) {
    ...
    const entries = slots.flatMap((value) => {
        ...
        return (id === "hero" || id === "portrait" || id === "detail") && source ? [[id, source]] : [];
    });
...
function section(page, key) {
    ...
    if (!found)
        throw new Error(`MUSICRAUM_RENDER_CONTRACT: missing ${key} section`);
    return found;
}
function navigationLabels(value) {
    const result = Object.fromEntries(SECTION_KEYS.map((key) => [key, ""]));
...
function isSectionKey(value) { return SECTION_KEYS.includes(value); }
function isPreset(value) { return value in PRESETS; }
function isFontSize(value) { return value in FONT_SIZES; }
