import { FONT_PRESETS, SCHEMA_VERSION } from "./domain.js";
import { evaluateReadiness } from "./readiness.js";
import { RAUM_FUER_KLANG_MEDIA, RAUM_FUER_KLANG_URL } from "./website-media.js";
export const MUSICRAUM_ADAPTER_ID = "ch.gasserwerk.musicraum";
export const MUSICRAUM_ADAPTER_VERSION = "1.0.0";
export const BUILD_EDIT_CORE_SCHEMA_VERSION = 2;
...
export function adaptMusicraumDraft(draft) {
    const source = draft;
    ...
    const diagnostics = readiness.results.map((result) => ({
...
function contentSection(draft, section) {
    const common = {
...
function targetId(target) {
    ...
    if (target.kind === "offer")
        return `offer:${target.offerId}:${target.field}`;
    if (target.kind === "text-item")
        return `text-item:${target.list}:${target.itemId}`;
...
function pathForTarget(target) {
    if (target.kind === "field")
        return target.field.split(".");
    if (target.kind === "offer")
        return ["offers", target.offerId, target.field];
    if (target.kind === "text-item")
        return [target.list, target.itemId];
    if (target.kind === "section")
        return ["layout", "visibility", target.section];
