import { escapeAttr, escapeHtml, normalizeEmail, normalizePhone, safeJson } from "./domain.js";
import { HARFE_FAVICON, RAUM_FUER_KLANG_MEDIA, RAUM_FUER_KLANG_URL } from "./website-media.js";
...
import { websiteModelFromDraft } from "./website-model.js";
const SECTION_META = {
...
export function renderMusicraumWebsiteHtml(draft, options = {}) {
    ...
    const nav = visibleOrder.map((key) => {
        const copyKey = NAV_COPY_KEYS[key];
        const field = `copy.${copyKey}`;
        const label = String(draft.copy[copyKey] ?? "").trim();
