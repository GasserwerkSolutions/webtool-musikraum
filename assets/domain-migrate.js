import { MAX_TESTIMONIALS, PRESETS, createClientId, createDefaultDraft, } from "./domain-model.js";
...
export function migrateV1ToV2(input, now = new Date().toISOString()) {
    if (input.version != null && input.version !== 1)
        throw new Error(`UNSUPPORTED_LEGACY_SCHEMA:${String(input.version)}`);
    ...
    rawServices.forEach((value, index) => {
        ...
        let slug = slugify(asString(row.id, asString(row.name, `service-${index + 1}`)));
        let suffix = 2;
        const base = slug;
    ...
    const dayByName = { Sonntag: 0, Montag: 1, Dienstag: 2, Mittwoch: 3, Donnerstag: 4, Freitag: 5, Samstag: 6 };
    const migratedHours = [0, 1, 2, 3, 4, 5, 6].map((dayOfWeek) => {
        const value = rawHours.find((item) => dayByName[asString(asRecord(item).day)] === dayOfWeek);
        if (!value)
            return structuredClone(fallback.businessHours.find((item) => item.dayOfWeek === dayOfWeek));
        const row = asRecord(value);
    ...
    return normalizeDraftV2({ ...fallback, draftId: createClientId("draft"), createdAt: now, updatedAt: now,
        ...
        copy: { ...fallback.copy, ...Object.fromEntries(Object.keys(fallback.copy).map((key) => [key, asString(copy[key], fallback.copy[key])])) },
