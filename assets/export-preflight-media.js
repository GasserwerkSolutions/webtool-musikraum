...
import { RAUM_FUER_KLANG_MEDIA } from "./website.js";
...
export class ExportPreflightController {
    ...
    setPanelVisible(visible) {
        ...
        if ((this.stateValue.status === "ready" || this.stateValue.status === "preparing") && this.stateValue.revision === this.options.readRevision())
            return;
    ...
    async prepare() {
        ...
        try {
            ...
            let preparedMedia;
            try {
                preparedMedia = await this.prepareMedia(controller.signal);
            }
            catch (error) {
                if (isAbortError(error) || !this.isCurrent(generation, revision, controller.signal))
                    return this.stateValue;
                if (this.isCurrent(generation, revision, controller.signal))
                    this.setState({ status: "failed", generation, revision, message: error instanceof Error ? error.message : "Die Bilder konnten nicht vorbereitet werden." });
                return this.stateValue;
            }
            if (!this.isCurrent(generation, revision, controller.signal))
                return this.stateValue;
    ...
    async prepareMedia(signal) {
        const entries = Object.entries(this.mediaAssets);
        const prepared = await Promise.all(entries.map(async ([key, source]) => {
            try {
                return [key, await fetchWebsiteMediaAsset(source, this.fetchAsset, signal, { timeoutMs: this.assetTimeoutMs }), true];
            }
            catch (error) {
                ...
                return [key, source, isDataImageUrl(source)];
            }
        }));
        const totalBytes = prepared.reduce((sum, [, source, embedded]) => sum + (embedded ? dataImageByteSize(source) : 0), 0);
        if (totalBytes > this.totalAssetMaxBytes)
            throw new ExportAssetError("total-size", `Die eingebetteten Bilder überschreiten zusammen ${Math.round(this.totalAssetMaxBytes / 1024 / 1024)} MiB.`);
        return {
            assets: Object.fromEntries(prepared.map(([key, source]) => [key, source])),
...
export async function fetchWebsiteMediaAsset(source, fetchAsset, signal, options = {}) {
    ...
    try {
        ...
        try {
            ...
            if (signal.aborted)
                throw abortError();
            if (timedOut)
                throw new ExportAssetError("timeout", "Ein Bild hat nicht innerhalb von 8 Sekunden geantwortet.");
            throw new ExportAssetError("network", error instanceof Error ? error.message : "Ein Bild konnte nicht geladen werden.");
        }
        if (!response.ok)
            throw new ExportAssetError("http", `Ein Bild antwortete mit HTTP ${response.status}.`);
        ...
        if (Number.isFinite(declaredSize) && declaredSize > maxBytes)
            throw new ExportAssetError("size", "Ein Bild überschreitet die Grenze von 5 MiB.");
        let blob;
        try {
...
function dataImageByteSize(value) {
...
function isDataImageUrl(value) { return /^data:image\/[a-z0-9.+-]+(?:;[^,]*)?,/i.test(value); }
async function blobToDataUrl(blob, mime) {
    if (typeof FileReader !== "undefined")
        return await new Promise((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = () => reject(new ExportAssetError("read", "Ein Bild konnte nicht in die Exportdatei geschrieben werden.")); reader.readAsDataURL(blob); });
