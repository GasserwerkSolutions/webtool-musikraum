...
export class BuilderStore {
    ...
    mutate(mutator) { const next = cloneDraft(this.draft); mutator(next); next.updatedAt = new Date().toISOString(); this.draft = normalizeDraft(next); this.emit(); this.scheduleSave(); }
    replace(next, persist = true) { this.draft = normalizeDraft(next); this.emit(); if (persist)
        this.scheduleSave(0); }
