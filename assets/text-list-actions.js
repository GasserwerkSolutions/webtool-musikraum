...
export function handleTextListAction(context, button) {
    const action = button.dataset.action;
    if (action === "add-text-item") {
        const list = parseList(button.dataset.list);
        if (!list)
            return true;
        if (context.store.snapshot[list].length >= MAX_TEXT_ITEMS) {
    ...
    if (action !== "remove-text-item")
        return false;
    const card = button.closest("[data-text-item-card]");
    const list = parseList(card?.dataset.textList);
    const itemId = card?.dataset.textItemId;
    if (!list || !itemId)
        return true;
    const item = context.store.snapshot[list].find((entry) => entry.id === itemId);
    if (!item)
        return true;
...
export function handleTextListInput(context, target) {
    if (!target.matches("[data-text-item-field]"))
        return false;
    const card = target.closest("[data-text-item-card]");
    const list = parseList(card?.dataset.textList);
    const itemId = card?.dataset.textItemId;
    if (!list || !itemId)
        return true;
    ...
    if (number)
        number.textContent = `${index + 1}. ${target.value || "Punkt"}`;
    return true;
}
