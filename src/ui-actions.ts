...
function addOffer(context: UiContext): void { if (context.store.snapshot.offers.length >= MAX_OFFERS) { showToast(`Du kannst höchstens ${MAX_OFFERS} Klangmomente anlegen.`); return; } context.store.mutate((draft) => { draft.offers.push({ id: createId("offer"), title: "Neuer Klangmoment", text: "" }); }); renderOffers(context); }
...
async function restoreBackup(context: UiContext, file: File): Promise<void> { try { if (!isBackupFileSizeAllowed(file)) { showToast("Diese Sicherung ist zu gross. Bitte verwende eine Musikraum-Sicherung unter 1 MB."); return; } const parsed: unknown = JSON.parse(await file.text()); const imported = normalizeDraft(parsed); await context.store.flush(); const restored = await replaceWithImportedDraft(context.repository, context.store.snapshot as MusicraumDraft, imported); context.store.replace(restored, false, false); bindStaticInputs(context); renderDynamicControls(context); renderPreview(context); updateReadiness(context); showPanel(context, "site"); showToast("Sicherung wiederhergestellt. Prüfe kurz die Vorschau."); } catch (error) { console.error(error); showToast("Diese Datei ist keine gültige Musikraum-Sicherung."); } finally { context.backupInput.value = ""; } }
