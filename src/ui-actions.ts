...
export function handleClick(context: UiContext, event: Event): void {
  ...
  if (panelButton) { showPanel(context, panelButton.dataset.panelTarget ?? "site"); return; }
  ...
  const layoutButton = target.closest<HTMLElement>("[data-layout-action]");
  if (layoutButton) { moveSection(context, layoutButton); return; }
  ...
  if (action === "copy-json") void copyDraftData(context);
...
export function handleInput(context: UiContext, event: Event): void {
  ...
  if (visibilityToggle instanceof HTMLInputElement) {
    const key = visibilityToggle.closest<HTMLElement>("[data-section-key]")?.dataset.sectionKey as SectionKey | undefined;
    if (!key) return;
  ...
  const serviceField = target.dataset.serviceField as "category" | "name" | "description" | undefined;
  ...
  if (serviceField && serviceCard?.dataset.serviceId) {
    context.store.mutate((draft) => {
      ...
      if (serviceField === "name") service.slug = uniqueSlug(target.value, draft.services, service.clientId);
    });
    if (serviceField === "name") {
      ...
      if (number) number.textContent = `${index + 1}. ${target.value || "Angebot"}`;
    }
  }
}

function moveSection(context: UiContext, button: HTMLElement): void {
  const key = button.closest<HTMLElement>("[data-section-key]")?.dataset.sectionKey as SectionKey | undefined;
  ...
  context.store.mutate((draft) => {
    ...
    if (index < 0 || nextIndex < 0 || nextIndex >= draft.layout.order.length) return;
...
function addService(context: UiContext): void {
  context.store.mutate((draft) => {
    ...
    const service: BuilderService = { clientId, slug: uniqueSlug("Neues Angebot", draft.services), category: "Klangabend", name: "Neues Angebot", description: "", durationMinutes: 60, price: 0, priceType: "on-request", bookable: true };
...
async function exportHtml(context: UiContext): Promise<void> {
  ...
  try { heroImageUrl = await fetchAsDataUrl(MUSICRAUM_HERO_URL); }
  catch (error) { console.warn("Das Titelbild konnte nicht eingebettet werden; der Export verwendet die feste Online-Quelle.", error); }
  const html = buildWebsiteHtml(context.store.snapshot as BuilderDraftV2, { heroImageUrl });
...
async function copyDraftData(context: UiContext): Promise<void> {
  try { await navigator.clipboard.writeText(JSON.stringify(context.store.snapshot, null, 2)); showToast("Der Musikraum-Entwurf wurde kopiert."); }
  catch { showToast("Kopieren wurde vom Browser blockiert."); }
}

async function resetBuilder(context: UiContext): Promise<void> {
  if (!window.confirm("Alle lokalen Änderungen verwerfen und den Musikraum-Entwurf zurücksetzen?")) return;
  try {
