import { cloneDraft, normalizeEmail, normalizeInstagramUrl, normalizePhone, type MusicraumDraft, type SectionKey } from "./domain.js";
...
export function draftsEqualIgnoringUpdatedAt(left: Readonly<MusicraumDraft>, right: Readonly<MusicraumDraft>): boolean {
...
export function createDraftEffect(before: Readonly<MusicraumDraft>, after: Readonly<MusicraumDraft>, intent: DraftMutationIntent): DraftEffect {
  ...
  if (intent.type === "set-text-item") {
    ...
    const expected = cloneDraft(before); const expectedItem = expected[intent.list].find((item) => item.id === intent.itemId); if (!expectedItem) throw new Error("INVALID_TEXT_ITEM_SET"); expectedItem.text = next.text;
  ...
  if (intent.type === "set-offer-field") {
    ...
    const expected = cloneDraft(before); const expectedOffer = expected.offers.find((offer) => offer.id === intent.offerId); if (!expectedOffer) throw new Error("INVALID_OFFER_FIELD_SET"); expectedOffer[intent.field] = next[intent.field];
  ...
  if (intent.type === "set-section-visibility") {
    ...
    const expected = cloneDraft(before); expected.layout.visibility[intent.section] = nextVisible; assertExpectedDraft(expected, after, "UNEXPECTED_SECTION_VISIBILITY_CHANGE");
  ...
  if (intent.type === "move-section") {
    ...
    const expected = cloneDraft(before); expected.layout.order.splice(previousIndex, 1); expected.layout.order.splice(nextIndex, 0, intent.section); assertExpectedDraft(expected, after, "UNEXPECTED_SECTION_MOVE_CHANGE");
  ...
  if (intent.type === "set-theme") {
    ...
    const expected = cloneDraft(before); expected.theme = { ...after.theme }; assertExpectedDraft(expected, after, "UNEXPECTED_THEME_CHANGE");
...
function createInsertEffect(before: Readonly<MusicraumDraft>, after: Readonly<MusicraumDraft>, collection: ContentCollection, itemId: string): DraftEffect {
  const previousItems = before[collection]; const nextItems = after[collection]; const index = nextItems.findIndex((item) => item.id === itemId);
  ...
  const withoutInserted = nextItems.filter((item) => item.id !== itemId); if (JSON.stringify(withoutInserted) !== JSON.stringify(previousItems)) throw new Error("UNEXPECTED_COLLECTION_INSERT_CHANGE");
  const expected = cloneDraft(before); expected[collection] = cloneDraft(after)[collection]; assertExpectedDraft(expected, after, "UNEXPECTED_COLLECTION_INSERT_DRAFT_CHANGE");
...
function createRemoveEffect(before: Readonly<MusicraumDraft>, after: Readonly<MusicraumDraft>, collection: ContentCollection, itemId: string): DraftEffect {
  const previousItems = before[collection]; const nextItems = after[collection]; const previousIndex = previousItems.findIndex((item) => item.id === itemId);
  ...
  const withoutRemoved = previousItems.filter((item) => item.id !== itemId); if (JSON.stringify(withoutRemoved) !== JSON.stringify(nextItems)) throw new Error("UNEXPECTED_COLLECTION_REMOVE_CHANGE");
  const expected = cloneDraft(before); expected[collection] = cloneDraft(after)[collection]; assertExpectedDraft(expected, after, "UNEXPECTED_COLLECTION_REMOVE_DRAFT_CHANGE");
...
function createMoveEffect(before: Readonly<MusicraumDraft>, after: Readonly<MusicraumDraft>, collection: ContentCollection, itemId: string): DraftEffect {
  const previousIndex = before[collection].findIndex((item) => item.id === itemId); const nextIndex = after[collection].findIndex((item) => item.id === itemId);
  if (previousIndex < 0 || nextIndex < 0 || previousIndex === nextIndex || before[collection].length !== after[collection].length) throw new Error("INVALID_COLLECTION_MOVE");
  const expected = cloneDraft(before); const [moved] = expected[collection].splice(previousIndex, 1); if (!moved) throw new Error("INVALID_COLLECTION_MOVE"); expected[collection].splice(nextIndex, 0, moved);
...
function assertOnlyFieldChanged(before: Readonly<MusicraumDraft>, after: Readonly<MusicraumDraft>, field: StaticEditableField): void {
  ...
  const expected = cloneDraft(before); setFieldValue(field, expected, nextValue); assertExpectedDraft(expected, after, "UNEXPECTED_FIELD_CHANGE");
}
...
function comparableDraft(draft: Readonly<MusicraumDraft>): MusicraumDraft { const copy = cloneDraft(draft); copy.updatedAt = ""; return copy; }
...
function intentIdentity(intent: DraftMutationIntent): string { if (intent.type === "set-field") return `${intent.type}:${intent.field}`; if (intent.type === "set-text-item") return `${intent.type}:${intent.list}:${intent.itemId}`; if (intent.type === "set-offer-field") return `${intent.type}:${intent.offerId}:${intent.field}`; if (intent.type.includes("collection-item")) return `${intent.type}:${intent.collection}:${intent.itemId}`; if (intent.type === "set-section-visibility" || intent.type === "move-section") return `${intent.type}:${intent.section}`; if (intent.type === "replace-draft") return `${intent.type}:${intent.reason}`; return intent.type; }
