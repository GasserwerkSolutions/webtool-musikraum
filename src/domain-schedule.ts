import type { DayOfWeek, TimeRange, WeeklySchedule } from "./domain-model.js";
...
function replaceDay(schedule: WeeklySchedule, dow: DayOfWeek, updater: (day: WeeklySchedule[number]) => WeeklySchedule[number]): WeeklySchedule {
...
export function setDayClosed(schedule: WeeklySchedule, dow: DayOfWeek, closed: boolean): WeeklySchedule {
  return replaceDay(schedule, dow, (day) => {
    if (closed) return { ...day, closed: true, ranges: [] };
...
export function setRangeField(schedule: WeeklySchedule, dow: DayOfWeek, index: number, field: "from" | "to", value: string): WeeklySchedule {
  return replaceDay(schedule, dow, (day) => {
    if (index < 0 || index >= day.ranges.length) return day;
...
export function addRange(schedule: WeeklySchedule, dow: DayOfWeek): WeeklySchedule {
  return replaceDay(schedule, dow, (day) => {
    if (day.ranges.length >= MAX_RANGES_PER_DAY) return day;
...
export function removeRange(schedule: WeeklySchedule, dow: DayOfWeek, index: number): WeeklySchedule {
  return replaceDay(schedule, dow, (day) => {
    if (index < 0 || index >= day.ranges.length) return day;
    const ranges = cloneRanges(day.ranges.filter((_, i) => i !== index));
    if (!ranges.length) return { ...day, closed: true, ranges: [] };
    return { ...day, ranges };
  });
}

// Copy the source day's closed state and ranges (deep) onto every target day except the source itself.
export function copyDayToDays(schedule: WeeklySchedule, sourceDow: DayOfWeek, targetDows: DayOfWeek[]): WeeklySchedule {
  const source = schedule.find((day) => day.dayOfWeek === sourceDow);
  if (!source) return schedule;
  const targets = new Set(targetDows);
  return schedule.map((day) => {
    if (day.dayOfWeek === sourceDow || !targets.has(day.dayOfWeek)) return day;
